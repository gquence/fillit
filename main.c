...
int	get_tetra_id(unsigned short int number, size_t prev_bb, size_t prev_hb, size_t b_border, size_t h_border)
{
	...
	if (tetra_value[pos] < number)
		return (get_tetra_id(number, b_border, h_border, pos, h_border));
	if (tetra_value[pos] > number)
		return (get_tetra_id(number, b_border, h_border, b_border, pos));
	return (pos);
}
...
int		read_one_tetro(const int fd)
{
	...
	ft_strcat(line, str);
	line[16] = 0;
	if (ft_stoi_bit(line, &buf) == -1);
		return (-1);
	return (get_tetra_id(buf, 0, 0, 0, 20));
}
...
int main(int ac, char **av)
{
	...
	if ((fd = open(av[1], O_RDONLY)) < 0)
		return (-2);
	printf("answer = %d", read_one_tetro(fd));
